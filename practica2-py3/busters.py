...
class GameState:
    ...
    def generateSuccessor(self, agentIndex, action):
        ...
        state.data.ghostDistances = [
            getNoisyDistance(
                p, state.getGhostPosition(i)) for i in range(
                1, state.getNumAgents())]
        state.ghostPositions = self.ghostPositions = [
            self.getGhostPosition(i) for i in range(
                1, self.getNumAgents())]
        a = 0
    ...
    def getGhostDirection(self, agentIndex):
        if agentIndex == 0:
            raise QLException("Pacman's index passed to getGhostDirection")
