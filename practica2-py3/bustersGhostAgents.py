...
class DispersingGhost(ghostAgents.GhostAgent):
    ...
    def getDistribution(self, state):
        ...
        for a in others:
            assert state.getGhostState(a) is not None, "Ghost position unspecified in state!"
        otherGhostPositions = [state.getGhostPosition(
            a) for a in others if state.getGhostPosition(a)[1] > 1]

        # for each action, get the sum of inverse squared distances to the other ghosts
        sumOfDistances = []
        for pos in newPositions:
            sumOfDistances.append(sum([(1 + manhattanDistance(pos, g))**(-2)
